	...
	return (
		<>
			...
			<main className='px-14 py-7 flex flex-col gap-8'>
				<h2 className='text-zinc-100 text-4xl font-normal'>Search &quot;{searchParams.query}&quot;</h2>
				<div className='flex flex-wrap gap-x-2 gap-y-12 overflow-x-scroll scrollbar-hide rounded-md w-[1920px]'>
					{movies?.results.map((movie) => (
						<MovieItem key={movie.id} data={movie} movieGenres={movieGenres.genres} moviesOnList={moviesOnList} />
					))}
					{shows?.results.map((movie) => (
						<MovieItem key={movie.id} data={movie} showGenres={showGenres.genres} showsOnList={showsOnList} />
					))}
				</div>
			</main>
			<Footer />
